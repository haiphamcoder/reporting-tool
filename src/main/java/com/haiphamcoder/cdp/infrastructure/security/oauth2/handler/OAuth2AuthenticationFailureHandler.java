    ...
    protected String determineTargetUrl(HttpServletRequest request, AuthenticationException exception) {
        String targetUrl = CookieUtils.getCookie(request, OAuth2AuthorizationRequestParams.FAILURE_REDIRECT_URL)
                .orElseThrow(() -> new IllegalArgumentException("Request param "
                        + OAuth2AuthorizationRequestParams.FAILURE_REDIRECT_URL + " is required and cannot be empty"))
                .getValue();
        String errorCode = "error.unknownError";
        if (exception instanceof OAuth2AuthenticationProcessingException) {
            errorCode = ((OAuth2AuthenticationProcessingException) exception).getErrorCode();
        }
